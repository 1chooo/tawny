...
import { PostsSearch } from "@/components/art/posts/posts-search";
import { getAllPosts, getAllTags, type PostSummary } from "@/lib/art/posts";
    ...
    <div className="bg-art-paper w-full px-5 pb-24 pt-10 md:px-8 md:pt-16">
      <div className="mx-auto max-w-3xl">
        <header className="border-art-border mb-8 border-b pb-6 md:mb-10 md:pb-8">
          ...
          <p className="text-art-ink-muted mb-2 text-sm md:text-base">
            {t("description")}
          </p>
        ...
        <div className="mb-8 space-y-6">
          <Suspense
            fallback={
              <div
                className="border-art-border bg-art-surface h-11 w-full animate-pulse rounded-lg border"
                aria-hidden
              />
            }
          >
        ...
        {filtered.length === 0 ? (
          ...
          <div className="space-y-12">
            {grouped.map(([year, posts]) => (
              <section key={year}>
                <h2 className="font-(family-name:--font-serif-display) text-art-ink-muted mb-6 text-2xl font-bold md:mb-6">
                  {year}
                </h2>
                <ul className="flex flex-col">
                  {posts.map((post) => (
                    <li
                      key={post.slug}
                      className="border-art-border flex flex-col border-b py-6 last:border-b-0 md:py-8"
                    >
                      <p className="text-art-ink-muted order-2 mb-2 text-sm md:order-1">
                        {formatDate(post.date, locale)}
                      </p>
                      <Link
                        href={`/notes/${post.slug}`}
                        className="font-(family-name:--font-serif-display) text-art-ink hover:text-art-accent order-1 text-2xl font-bold leading-tight transition-colors md:order-2 md:text-3xl"
                      >
                        {post.title}
                      </Link>
                      <p className="text-art-ink-muted order-3 mt-3 text-base leading-relaxed">
                        {post.description}
                      </p>
                          ...
                          {post.tags.map((tg) => (
                            <li key={tg}>
                              <Link
                                ...
                                className="bg-art-accent-soft text-art-accent rounded-full px-2.5 py-1 text-xs font-medium tracking-wide uppercase hover:opacity-90"
                              >
                                {tg}
                              </Link>
                            </li>
