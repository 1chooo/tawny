    ...
    <div className="border-art-border my-8 rounded-lg border p-5">
      ...
      <ul className="space-y-3">
        {filtered.map((post) => (
          <li
            key={post.slug}
            className="border-art-border flex flex-wrap items-center justify-between gap-2 border-b pb-3 last:border-b-0 last:pb-0"
          >
            <Link
              href={`/notes/${post.slug}`}
              className="text-art-ink hover:text-art-accent font-medium transition-colors"
            >
              {post.title}
            </Link>
            <span className="text-art-ink-muted text-xs">
              {post.tags?.join(", ")}
            </span>
          </li>
        ))}
      ...
      {filtered.length === 0 ? (
        <p className="text-art-ink-muted mt-3 text-sm">{t.empty}</p>
      ) : null}
    </div>
  );
}
