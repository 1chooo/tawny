      ...
      <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
        {STACK.map((item, i) => (
          <button
            ...
            onClick={() => setActive(i)}
            className={clsx(
              "border-art-border rounded-lg border p-4 text-left transition-colors",
              active === i
                ? "border-art-accent bg-art-accent-soft/50"
                : "bg-art-surface hover:border-art-accent/50",
            )}
            aria-pressed={active === i}
          >
            <p className="text-art-ink text-sm font-bold md:text-base">{item.name}</p>
            <p className="text-art-ink-muted mt-1 text-xs">{item.role}</p>
          </button>
        ))}
      </div>
      <p className="text-art-ink-muted mt-4 text-base leading-relaxed">
        {STACK[active].detail}
      </p>
    </div>
  );
